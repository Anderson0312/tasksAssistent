...
const Task = ({ task, provided}: TaskProps) =>{
    ...
    return (
        ...
        className="w-full cursor-grap bg-white flex flex-col justify-between gap-3 items-start shadow-sm rounded-xl px-3 py-4"
        >
            {image && alt && (
            ...
            <div className="w-full flex items-center justify-between">
                <div className="flex items-center gap-1">
                    <TimeOutline color='#666' width='19px' height='19px'></TimeOutline>
                    <span className="text-[13px] text-gray-700">{deadline} Minutos</span>
                </div>
                <div className={`w-[60px] rounded-full h-[5px] ${
                    priority ==='hight'? 'bg-red-500':priority ==='medium'? 'bg-green-500':'bg-blue-500'
                }`}>
                </div>
            </div>
        </div>
